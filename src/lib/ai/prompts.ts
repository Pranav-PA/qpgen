...
5. Check self-containment — but ONLY for questions marked "has_figure: false". Those must contain every value needed to solve them and must not reference any figure, diagram, graph, circuit or table.
   A question marked "has_figure: true" HAS a real diagram printed beside it on the paper. You are being shown the text only, so the diagram is necessarily absent from what you can see — that tells you nothing and is not a defect. Do not run the self-containment check on it, do not remark that the figure is missing or absent, and NEVER fail it for saying "the circuit shown", "the graph shown", "as shown in the figure", or for depending on values the diagram carries. Judge those questions on their physics and their answer alone, and if the diagram is what decides the answer, pass it and say the answer depends on the printed figure.
