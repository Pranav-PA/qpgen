...
- "mcq": exactly 4 options; exactly one correct. Options must be plausible distractors reflecting common student errors — not obviously wrong fillers. Do not embed option letters in the option text.
...
- "assertion_reason": question_text contains "Assertion (A): …" and "Reason (R): …" on separate lines. Options are exactly: ["Both A and R are true and R is the correct explanation of A", "Both A and R are true but R is NOT the correct explanation of A", "A is true but R is false", "A is false but R is true"].

Solutions:
- Every question gets a complete step-by-step solution a student could learn from: state the concept/formula, show the working, end with why the correct option/value is right. For MCQs, briefly note why the key distractor is wrong when helpful.
